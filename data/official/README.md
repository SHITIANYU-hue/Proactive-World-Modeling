...
| `PIWM-Target-Frontcam-v1` | `piwm_target_v1` | `/Users/mutsumi/Desktop/WorkSpace/piwm` | target 域智能售货柜前置摄像头数据 | 118 records / 354 frames / 708 ms-swift examples；synthetic_unreviewed |
...
- `PIWM-Target-Frontcam-v1` 已写出到 `data/official/piwm_target_v1/`。它来自 `guochenmeinian/piwm` 的 118 条设备前置摄像头样本，使用 `target_frontcam` 和 `actor_profile=target_terminal_logic`。当前 QA 状态是 `synthetic_unreviewed`。
...
`piwm_train_target_specialization_v1.jsonl` 来自 `PIWM-Target-Frontcam-v1`，共 708 examples：perception 118 / deliberation 472 / action_selection 118。它适合作为 target-domain specialization pilot 的 Stage-2 数据入口，但当前 target test split 仍需人工 QA，不能直接声称为 QA-reviewed in-domain benchmark。
...
- 写 target 域设备前置摄像头数据时，引用 `PIWM-Target-Frontcam-v1`；不要把它写成 QA-reviewed。
...
- `PIWM-Target-Frontcam-v1` 在人工 QA 完成前，不得写成正式 in-domain eval；当前 30 条 test split 只是 unreviewed eval candidate。
