...
def test_all_tag_literals_are_lowercase_and_closed() -> None:
    for group in (config.PERCEPTION_TAGS, config.DELIBERATION_TAGS, config.CONTINUATION_TAGS, config.ACTION_TAGS):
        for tag in group:
...
def test_tag_instruction_lines_uses_pairs_in_order() -> None:
    ...
    assert lines[-1] == f"{config.TAG_CANDS_OPEN}...{config.TAG_CANDS_CLOSE}"
